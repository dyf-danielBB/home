...
const blogConfigurationFiles = new Set(['.dockerignore', 'Dockerfile', 'UPSTREAM.md', 'compose.yml']);
const upstreamBlobs = {
  ...
  'src/components/Header.astro': '0e899c37585113d82e4fdc34c91b4740fafd84a2',
  'src/components/Link.astro': '3ed382d64cba4a7d4d44e00206e330852db1048d',
  'src/consts.ts': '6a86c9f18a9f83a54502ab1af0fd2eb854b2d206',
  ...
  'src/layouts/Layout.astro': 'dd83848c876e7ac6130169517709bd3a1f032ecd',
  ...
  'src/styles/global.css': '59320c2b35e769e5da23d7f1bbf68df30fff77c2',
...
test('excludes local dependencies, build output, secrets, logs, and editor files from Docker', async () => {
  ...
  for (const pattern of ['.git', 'node_modules', 'dist', '.astro', '.env*', '*.log', '.vscode', '.idea']) {
    assert.ok(patterns.has(pattern), `missing Docker ignore pattern: ${pattern}`);
  }
});

test('preserves the complete 30-file upstream source tree offline', async () => {
  const sourceFiles = await listSourceFiles();

  assert.equal(Object.keys(upstreamBlobs).length, 30);
  assert.deepEqual(sourceFiles.sort(), Object.keys(upstreamBlobs).sort());

  for (const [file, expectedBlob] of Object.entries(upstreamBlobs)) {
    assert.equal(gitBlobHash(await readFile(new URL(file, blogDirectory))), expectedBlob, `${file} changed`);
  }
});
